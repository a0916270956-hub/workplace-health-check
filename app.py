...
【🚨 最高查證原則：必須使用最新法規】
1. 遇到使用者詢問具體法規、罰則、假別天數、給薪標準時，你 **必須使用 Google 搜尋工具** 查詢最新規定。
2. 搜尋時，請優先以台灣官方網站的資訊為準（如：全國法規資料庫 law.moj.gov.tw、勞動部官網 mol.gov.tw）。
3. 答覆時，請明確指出你引用的是「最新規定」（例如說明是依據民國幾年的修法）。
...
try:
    model = genai.GenerativeModel(
        ...
        # 🎯 關鍵修正：將字串改為「字典格式列表」，完美避開底層套件的字串檢查錯誤
        tools=[{"google_search": {}}] 
    )
except Exception as e:
