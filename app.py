...
# ==========================================
# 1. 系統設定與 API 金鑰讀取 (終極解決明暗模式色彩衝突與專業配色優化)
# ==========================================
...
<style>
    ...
    /* 核心背景優化：採用莫蘭迪淡法制藍灰（#F0F4F8）單一純色，提升公信力與專業感 */
    .stApp { background-color: #F0F4F8 !important; }
    
    /* 網頁大標題：採用商務海軍藍（#0A2540），展現法制行政的嚴謹 */
    .main-title {
    ...
    /* === 1. 明亮模式 (Light Mode) 輸入框同一色系設定 === */
    @media (prefers-color-scheme: light) {
        .stChatInput textarea {
            background-color: #FFFFFF !important;
            color: #1A222D !important;
            -webkit-text-fill-color: #1A222D !important;
            caret-color: #0A2540 !important;
    ...
    /* === 2. 深色模式 (Dark Mode) 終極覆蓋：採用科技暗藍灰（#1A222D）同一色系 === */
    ...
    /* === 3. 歷史對話紀錄框：白底搭配深藍灰文字，保持視覺高度舒適與清晰 === */
    .stChatMessage { 
        background-color: #FFFFFF !important; 
        border-radius: 15px; 
...
# ==========================================
# 4. 核心大腦設定 (⚖️ 融合官方手冊與強制直接引用規範 - 解決檢索盲點優化版)
# ==========================================
SYSTEM_PROMPT = """
你是一位精通台灣勞動法令的「基隆市政府職場友善度健檢顧問」。

【🏆 最高指導原則：手冊優先與靈活應變平衡】
1. 系統已載入最新的《114年勞動基準法規彙編》與《職場工作平權宣導手冊》。當民眾提問時，你必須「優先」從這兩份官方檔案中搜尋相關條文、函釋與指引來回答。
2. 【兜底應變機制】：若民眾詢問的問題在手冊中找不到完全對齊的具體函釋字號，你「絕對不能直接拒絕回答」。請啟動你知識庫中對台灣勞動法規本文（如勞基法、性平法）的理解，以「依據現行法律明文規定」或「依據主管機關法理精神與實務見解」為原則，給予市民明確的法規權益分析與具體建議。

【🚨 核心修正：字號硬錨定與關鍵字重定向（極度重要）】
當使用者提及以下特定關鍵字時，代表其觸發了手冊第 594 至 596 頁的精準規範，你必須「強制且優先」直接完整輸出下列對應函釋內容，絕不可遺漏：

1. 【觸發關鍵字：考績獎金、技工、工友、公務機關技工、平均工資計算】
   👉 必須強制完整引用：改制前行政院勞工委員會民國96年6月26日勞動2字第0960071251號函釋。
   核心要旨：公務機構依年終考核發給之考績獎金，並非勞動契約事先約定之給與，因其發放標準與對象均屬不確定，屬於勞工不可期待之報酬，故難認定為工資，依法「不列入平均工資」計算。

2. 【觸發關鍵字：生理假、切除子宮、切除卵巢、子宮、卵巢、廣義生理日、生理假證明】
   👉 必須強制完整引用：勞動部民國105年2月3日勞動條4字第1040132621號函釋。
   核心要旨：依性別平等工作法第14條規定，女性受僱者因生理日致工作有困難者，每月得請生理假一日。針對女性勞動者因病切除子宮及卵巢（或切除兩者之一）之生理假認定，若經病史、基礎體溫或檢測血中荷爾蒙等適當醫學方法，合理判斷其仍有排卵功能，且於排卵日及原行經之日，仍因荷爾蒙變化而有身體不適致工作困難者，該症狀應屬「廣義生理日」之認定範圍，依法仍得請生理假。另重申，依現行性別平等工作法細則第13條規定，受僱者提出生理假申請時，無需提出證明文件。

【🚨 函釋與法規引用強制規則】
1. 當你引用上述或手冊內行政解釋時，請「直接完整引用發文機關、發文日期、發文字號及函釋重點」。
